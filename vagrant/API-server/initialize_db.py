from myapp.models import Restaurant, MenuItem, User, Base
from myapp.controllers import session
...
def add():
    ...
    menu_item2 = MenuItem(user_id=1, name="Veggie Burger",
    ...
    menu_item1 = MenuItem(user_id=1, name="French Fries", description="with garlic and parmesan",
                         price="$2.99", course="Appetizer", restaurant=restaurant1)

    session.add(menu_item1)
    session.commit()

    menu_item2 = MenuItem(user_id=1, name="Chicken Burger",
    ...
    menu_item3 = MenuItem(user_id=1, name="Chocolate Cake", description="fresh baked and served with ice cream",
                         price="$3.99", course="Dessert", restaurant=restaurant1)

    session.add(menu_item3)
    session.commit()

    menu_item4 = MenuItem(user_id=1, name="Sirloin Burger", description="Made with grade A beef",
                         price="$7.99", course="Entree", restaurant=restaurant1)

    session.add(menu_item4)
    session.commit()

    menu_item5 = MenuItem(user_id=1, name="Root Beer", description="16oz of refreshing goodness",
                         price="$1.99", course="Beverage", restaurant=restaurant1)

    session.add(menu_item5)
    session.commit()

    menu_item6 = MenuItem(user_id=1, name="Iced Tea", description="with Lemon",
                         price="$.99", course="Beverage", restaurant=restaurant1)

    session.add(menu_item6)
    session.commit()

    menu_item7 = MenuItem(user_id=1, name="Grilled Cheese Sandwich",
    ...
    menu_item8 = MenuItem(user_id=1, name="Veggie Burger",
    ...
    menu_item1 = MenuItem(user_id=1, name="Chicken Stir Fry",
    ...
    menu_item2 = MenuItem(user_id=1, name="Peking Duck",
                         description=" A famous duck dish from Beijing[1] "
                                     ...
                                     "mostly the skin and little meat, sliced in front of "
    ...
    menu_item3 = MenuItem(user_id=1, name="Spicy Tuna Roll",
    ...
    menu_item4 = MenuItem(user_id=1, name="Nepali Momo ",
    ...
    menu_item5 = MenuItem(user_id=1, name="Beef Noodle Soup",
    ...
    menu_item6 = MenuItem(user_id=1, name="Ramen",
                         description="a Japanese noodle soup dish. "
                                     "It consists of Chinese-style wheat noodles served in a meat"
    ...
    menu_item1 = MenuItem(user_id=1, name="Pho",
    ...
    menu_item2 = MenuItem(user_id=1, name="Chinese Dumplings",
                         description="a common Chinese dumpling which generally consists of minced meat and "
                                     "finely chopped vegetables wrapped into a piece of dough skin. "
    ...
    menu_item3 = MenuItem(user_id=1, name="Gyoza",
    ...
    menu_item4 = MenuItem(user_id=1, name="Stinky Tofu",
    ...
    menu_item1 = MenuItem(user_id=1, name="Tres Leches Cake",
    ...
    menu_item2 = MenuItem(user_id=1, name="Mushroom risotto",
    ...
    menu_item3 = MenuItem(user_id=1, name="Honey Boba Shaved Snow",
    ...
    menu_item4 = MenuItem(user_id=1, name="Cauliflower Manchurian",
    ...
    menu_item5 = MenuItem(user_id=1, name="Aloo Gobi Burrito",
    ...
    menu_item1 = MenuItem(user_id=1, name="Shellfish Tower",
    ...
    menu_item2 = MenuItem(user_id=1, name="Chicken and Rice", description="Chicken... and rice",
                         price="$4.95", course="Entree", restaurant=restaurant1)

    session.add(menu_item2)
    session.commit()

    menu_item3 = MenuItem(user_id=1, name="Mom's Spaghetti",
    ...
    menu_item4 = MenuItem(user_id=1, name="Choc Full O\' Mint (Smitten\'s Fresh Mint Chip ice cream)",
                         description="Milk, cream, salt, ..., Liquid nitrogen magic", price="$3.95", course="Dessert", restaurant=restaurant1)

    session.add(menu_item4)
    session.commit()

    menu_item5 = MenuItem(user_id=1, name="Tonkatsu Ramen",
    ...
    menu_item1 = MenuItem(user_id=1, name="Lamb Curry", description="Slow cook that thang in a pool of tomatoes, onions and alllll those tasty Indian spices. Mmmm.",
                         price="$9.95", course="Entree", restaurant=restaurant1)

    session.add(menu_item1)
    session.commit()

    menu_item2 = MenuItem(user_id=1, name="Chicken Marsala", description="Chicken cooked in Marsala wine sauce with mushrooms",
                         price="$7.95", course="Entree", restaurant=restaurant1)

    session.add(menu_item2)
    session.commit()

    menu_item3 = MenuItem(user_id=1, name="Potstickers", description="Delicious chicken and veggies encapsulated in fried dough.",
                         price="$6.50", course="Appetizer", restaurant=restaurant1)

    session.add(menu_item3)
    session.commit()

    menu_item4 = MenuItem(user_id=1, name="Nigiri Sampler", description="Maguro, Sake, Hamachi, Unagi, Uni, TORO!",
                         price="$6.75", course="Appetizer", restaurant=restaurant1)

    session.add(menu_item4)
    session.commit()

    menu_item2 = MenuItem(user_id=1, name="Veggie Burger", description="Juicy grilled veggie patty with tomato mayo and lettuce",
    ...
    menu_item9 = MenuItem(user_id=1, name="Chicken Fried Steak",
    ...
    menu_item1 = MenuItem(user_id=1, name="Boysenberry Sorbet", description="An unsettlingly huge amount of ripe berries turned into frozen (and seedless) awesomeness",
                         price="$2.99", course="Dessert", restaurant=restaurant1)

    session.add(menu_item1)
    session.commit()

    menu_item2 = MenuItem(user_id=1, name="Broiled salmon", description="Salmon fillet marinated with fresh herbs and broiled hot & fast",
                         price="$10.95", course="Entree", restaurant=restaurant1)

    session.add(menu_item2)
    session.commit()

    menu_item3 = MenuItem(user_id=1, name="Morels on toast (seasonal)",
    ...
    menu_item4 = MenuItem(user_id=1, name="Tandoori Chicken", description="Chicken marinated in yoghurt and seasoned with a spicy mix(chilli, tamarind among others) and slow cooked in a cylindrical clay or metal oven which gets its heat from burning charcoal.",
    ...
    menu_item10 = MenuItem(user_id=1, name="Spinach Ice Cream", description="vanilla ice cream made with organic spinach leaves",
                          price="$1.99", course="Dessert", restaurant=restaurant1)

    session.add(menu_item10)
    session.commit()
    ...
    menu_item1 = MenuItem(user_id=1, name="Super Burrito Al Pastor",
                         description="Marinated Pork, Rice, Beans, Avocado, Cilantro, Salsa, Tortilla", price="$5.95", course="Entree", restaurant=restaurant1)

    session.add(menu_item1)
    session.commit()

    menu_item2 = MenuItem(user_id=1, name="Cachapa", description="Golden brown, corn-based Venezuelan pancake; usually stuffed with queso telita or queso de mano, and possibly lechon. ",
                         price="$7.99", course="Entree", restaurant=restaurant1)
    ...
    menu_item1 = MenuItem(user_id=1, name="Chantrelle Toast", description="Crispy Toast with Sesame Seeds slathered with buttery chantrelle mushrooms",
                         price="$5.95", course="Appetizer", restaurant=restaurant1)

    session.add(menu_item1)
    session.commit()

    menu_item1 = MenuItem(user_id=1, name="Guanciale Chawanmushi",
    ...
    menu_item1 = MenuItem(user_id=1, name="Lemon Curd Ice Cream Sandwich",
